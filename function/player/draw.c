...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/23 23:25:08 by arthur            #+#    #+#             */
/*   Updated: 2021/06/24 11:09:35 by arthur           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_sprite(t_game *game)
{
	...
	if (game->map.player.jumping == 1)
		...
		type = 0;
	if (game->map.player.moving == 1 && game->map.player.jumping == 0
		&& game->map.player.falling == 0)
	{
...
void	draw_player_at(t_game *game, int x, int y)
{
	...
	while (xi < (WIDTH / CHUNCK_SIZE))
	{
		...
		{
			color = get_color(&game->map.player.textures[type].img.addr,
					game->map.player.textures[type].width, yi / 2, xi / 2);
			if (*(unsigned int *)color != 0)
			{
				if (x + xi >= 0 && x + xi <= WIDTH && y + yi >= 0 && y + yi
					<= WIDTH)
					pixel_put(&game->image, x + xi, y + yi,
						*(unsigned int *)color);
			}
			yi++;
		}
