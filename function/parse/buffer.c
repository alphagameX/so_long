...
void	is_valid_map(t_game *game, t_sarray *array, t_checking_map i)
{
	int	y;
	int	x;
...
{
	if (i.y == 0 || i.x == 0 || i.x == ft_strlen(array->buffer
	...
	if ((array->buffer[i.y + 1][i.x] == '1' || array->buffer
		[i.y + 1][i.x] == '0' || array->buffer[i.y + 1][i.x] == 'E'
		|| array->buffer[i.y + 1][i.x] == 'P' || array->buffer[i.y + 1]
		[i.x] == 'C' || array->buffer[i.y + 1][i.x] == '#') && (array->buffer
		[i.y - 1][i.x] == '1' || array->buffer[i.y - 1][i.x] == '0'
		|| array->buffer[i.y - 1][i.x] == 'E' || array->buffer[i.y - 1]
		[i.x] == 'P' || array->buffer[i.y - 1][i.x] == 'C' || array->buffer
		[i.y - 1][i.x] == '#') && (array->buffer[i.y][i.x + 1] == '1'
		|| array->buffer[i.y][i.x + 1] == '0' || array->buffer[i.y][i.x + 1]
		== 'E' || array->buffer[i.y][i.x + 1] == 'P' || array->buffer[i.y]
		[i.x + 1] == 'C' || array->buffer[i.y][i.x + 1] == '#')
		&& (array->buffer[i.y][i.x - 1] == '1' || array->buffer[i.y]
		[i.x - 1] == '0' || array->buffer[i.y][i.x - 1] == 'E'
		|| array->buffer[i.y][i.x - 1] == 'P' || array->buffer[i.y]
		[i.x - 1] == 'C' || array->buffer[i.y][i.x - 1] == 'P'))
	{
...
void 	checking_map(t_sarray *array, t_game *game)
{
	...
	while (i.y < array->count)
	{
		...
		{
			if (array->buffer[i.y][i.x] == '0' || array->buffer[i.y][i.x] == 'E'
				|| array->buffer[i.y][i.x] == 'P' || array->buffer[i.y]
				[i.x] == 'C' || array->buffer[i.y][i.x] == '#')
			{
