...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/24 13:25:25 by arthur            #+#    #+#             */
/*   Updated: 2021/06/24 18:30:13 by arthur           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_texture	*load_texture(t_game *game, char **paths, int count)
{
	...
	while (i < count)
	{
		...
		textures[i].img.addr = mlx_get_data_addr(textures[i].img.img,
				&textures[i].img.bits_per_pixel,
				&textures[i].img.line_length,
				&textures[i].img.endian);
		i++;
	}
